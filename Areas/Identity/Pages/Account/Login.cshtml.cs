    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                string signInWith = Input.EmailOrUserName;
                var user = await _userManager.FindByEmailAsync(Input.EmailOrUserName);

                if (user != null)
                {
                    signInWith = user.UserName;
                }

                var result = await _signInManager.PasswordSignInAsync(signInWith, Input.Password, Input.RememberMe, lockoutOnFailure: true);
                if (result.Succeeded)
                {
